...
def _admin_required(user):
    return user.is_authenticated and (
        getattr(user, "is_owner", False) or user.is_staff
    )
...
def xhttp_devices_view(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                xhttp_device = XHTTPDeviceService.create_device(
                    ...
                    performance_profile=form.cleaned_data["performance_profile"],
                    actor=request.user,
                )
                messages.success(
                    request,
                    (
                        ...
                        f"с профилем {xhttp_device.get_performance_profile_display()}."
                    ),
                )
                ...
                messages.error(
                    request,
                    f"Не удалось создать XHTTP-подключение: {exc}",
                )
    else:
        initial = {}
        ...
        server = Server.objects.filter(is_enabled=True).order_by("pk").first()
    ...
    devices = (
        XHTTPDevice.objects
        .select_related("device", "device__account", "server")
...
def xhttp_device_download_view(request, pk: int):
    device = get_object_or_404(
        XHTTPDevice.objects.select_related("device", "device__account", "server"),
        pk=pk,
    )
    ...
    owner_name = f"{device.device.account.display_name}-{device.device.name}"
    filename_base = slugify(f"{owner_name}-{device.name}") or f"xhttp-device-{device.id}"
...
def xhttp_device_action_view(request, pk: int, action: str):
    ...
    actions = {
        ...
        "check": (
            ...
            "UUID присутствует в Xray, конфигурация корректна.",
        ),
    }
