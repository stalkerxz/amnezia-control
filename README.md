...
- `docker inspect`, live-конфиги и `wg/awg show dump` помечены как sensitive runtime output: их command/stdout/stderr не сохраняются в JobEvent;
...
## Напоминания администраторам об истечении VPN-клиентов
Администраторы могут получать одно сгруппированное уведомление о VPN-клиентах, срок действия которых скоро закончится. Логика выбирает только активных клиентов с заполненным `expires_at`, не включает уже истёкших клиентов и сохраняет факт отправки в базе, чтобы не слать дубли для той же пары клиент/порог/дата истечения. Если клиенту продлили срок и `expires_at` изменился, напоминание для новой даты может быть отправлено снова.
...
- `EXPIRATION_REMINDER_CHANNELS` — каналы отправки через запятую: `email`, `telegram` или `email,telegram`. По умолчанию `email`. Для Telegram-only режима задайте `EXPIRATION_REMINDER_CHANNELS=telegram`.
- `ADMIN_EXPIRATION_REMINDER_EMAILS` — email-получатели через запятую. Если пусто, используется Django `ADMINS`, если он настроен.
- `TELEGRAM_BOT_TOKEN` — токен Telegram-бота от BotFather. Не храните токен в коде или репозитории.
- `TELEGRAM_ADMIN_CHAT_IDS` — chat id администраторов или групп через запятую, например `123456789,-1001234567890`.
- `SITE_URL` или `PUBLIC_BASE_URL` — базовый URL для абсолютных ссылок на карточки клиентов в уведомлении, например `SITE_URL=https://control.example.com`. Не задавайте production-домен в коде; используйте переменные окружения.
...
1. Создайте бота через BotFather и напишите ему любое сообщение от нужного администратора.
2. Откройте `https://api.telegram.org/bot<TELEGRAM_BOT_TOKEN>/getUpdates` и найдите поле `message.chat.id` в ответе. Не публикуйте URL с токеном и не коммитьте вывод API.
3. Для группового чата добавьте бота в группу, отправьте сообщение в группу и возьмите `chat.id` из `getUpdates`; у супергрупп id обычно начинается с `-100`.
